...
**4143 tests**, in 71 files, recounted from a `--collect-only` run on 2026-09-01, and equal to the `4143 passed` the gate reported on the same commit.

**The rows sum to 4132, eleven short, and the eleven are deliberate.** They belong to a test of a script under a directory the public mirror strips, so the file is on the `DENY` list and this register may not name it: a published file pointing at a stripped path fails the publish gate, which is how the omission was found rather than chosen. **The gap is stated rather than hidden**, which is the same arrangement the frontend register already uses for a different reason. Recount the headline with the command, and expect the rows to fall short by exactly that file. **The per file figures below are collected tests, so they sum to the headline exactly**, and that is the check: `pytest --collect-only -q` prints a count per file, and the total must equal the number above. They are not `def test_` lines, which come to 3448, because a parametrised case is one line and several tests, which is why the collected figure is the **larger** of the two. **That figure was 2788 and stale by 638 when this register was recounted for the wave that added the National Library of Greece**: the headline and every row were re-derived and the sentence stating what they are *not* was read past. Recount both, or the distinction this paragraph exists to make is drawn against a number from a different suite.
...
| `test_catalogue.py` | 56 | Folding what one source repeats, filling one row from another, merging two catalogues of one printing, how complete a record is, the two draft shapes, the headings a picked row confirms, that the fold runs once per set of collections, and that authority assertions follow the collection rules |
...
| `routers/test_books.py` | 110 | Listing, search, sorting, tagging, covers, notes, export, ownership |
