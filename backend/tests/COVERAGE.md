...
**8596 tests, in 124 files**, counted by the run that reads this line.
**The 114 rows below sum to 8190.**
The other 406 tests are in 10 files this register may not name.
<!-- measured: end -->
...
two destinations, a row here or the strip list, and one that takes neither fails by name.

**Collected is what the rows sum to; passed is what the gate prints.** They differ by the
skips plus the open recorded defects, both zero, and
`test_nothing_collected_is_skipped_or_left_expected_to_fail` is what keeps them so. **The
...
| `test_isbn.py` | 77 | Parsing, check digits, ISBN-10 to ISBN-13, the equivalent forms, and the same rules as properties over generated input: that a check digit is the only one that completes its body, that parsing is idempotent and answers None or thirteen ASCII Bookland digits, and that inserting any character `normalise` discards changes nothing |
...
| `test_bibliographic.py` | 88 | **What a bibliographic value means, with no record and no transport.** Every rule on its own, that the two language tables are one table inverted, and that only the four carrier aware doors reach the prose refusal, and that the catalogue order flip counts commas after the noise strip and takes a terminal full stop only on the branch that reorders a name, and, over generated input, that every rule here answers for any string at all, that a title is quoted from the record rather than rewritten, and that the disc half and the not a book half are still one refusal |
...
| `test_errors.py` | 42 | Content-negotiated errors, the 500 handler, API-vs-SPA routing, and that no hand raised refusal uses a status the schema declares with an array detail |
...
| `test_shelf.py` | 231 | The seam every many-book query goes through, and the only enforcement of the privacy rule since the AST guard was deleted. |
...
| `test_ratelimit.py` | 40 | The sliding window, and the login/registration limits |
...
| `test_house_rules.py`                      |   361 | **Defects a person found four times.** Every caller-supplied row id bounded at both ends, whether it arrives as a query parameter. Also the one decision of what vendored code is: each walk in the test tree that reaches that decision, driven against a constructed tree with each kind of vendored path planted in front of it, and no other test module allowed to define a walk of its own |
...
| `test_coverage_register.py` | 16 | **This register, against the run that reads it.** Every row's count and the generated block recomputed from the collection in hand, the partition that makes a file with no row one the publish gate strips rather than one nobody described, and the parser's own silent failures |
| `schemas/test_common.py` | 18 | **One line a member typed, and the one place the API layer spells it.** The three normalisations, the partition over the control characters that keeps the middle one from becoming the strictest, the ordering a URL bought, and the walk that refuses a whitespace split, and a control character in a docstring, in any module declaring a request contract or a route |
| `schemas/test_book.py` | 42 | **Two request bodies writing one column must agree about it.** `BookMatch` bounded four of its seventeen fields while `merge_into` wrote them all |
...
| `test_dialect.py` | 86 | One rule, two engines: that each dialect's arm renders where the rule is rendered and a third is refused there, that the SQLite arm of every ported constraint is the text it always was, and that no SQLite arm lost a clause the Postgres arm is allowed to omit, and that each revision's Postgres arm is the next one's starting point |
| `test_catalogue.py` | 237 | Folding what one source repeats, filling one row from another, merging two catalogues of one printing, how complete a record is, the two draft shapes, and what a source may not assert about this deployment's own files |
...
| `routers/test_collections.py` | 23 | **Shelving, never permission.** Naming a part of the shelf, the case-insensitive uniqueness the database enforces, in ASCII and outside it, counts filtered to the caller |
| `routers/test_books_collections.py` | 33 | Filing a book, the two list parameters and the 400 for both at once, the bulk verb, the merge that absorbs a collection, and the export column |
| `routers/test_books_duplicates.py` | 35 | Duplicate detection and the merge, incl. the ORM cascade trap |
...
| `routers/test_books.py`                    |   161 | Listing, search, sorting, tagging, covers, notes, export, ownership, that every cell the export writes goes through the formula escape with no exemption, and that a login this deployment holds leaves with the request that needs it |
...
| `routers/test_books_tags.py` | 38 | **Two vocabularies in one table.** Who may create, who may delete, and the counts |
| `routers/test_auth.py` | 84 | Registration, login, `/auth/me`, the registration switch, switching into a test account in all three modes, and that an address given at registration is stored, normalised |
