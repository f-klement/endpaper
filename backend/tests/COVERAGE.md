...
**3892 tests**, in 70 files, measured by the gate run on 2026-08-30. **The per file figures below are collected tests, so they sum to the headline exactly**, and that is the check: `pytest --collect-only -q` prints a count per file, and the total must equal the number above. They are not `def test_` lines, which come to 2788, because a parametrised case is one line and several tests.
...
| `test_sources.py` | 36 | **The provider roster.** That off means not asked rather than deprioritised, that the stored order is the order sources are asked and not which is believed, and that a library with every source off refuses rather than answering nothing |
