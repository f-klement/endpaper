...
**3644 tests**, in 66 files, measured by the gate run on 2026-08-29. **The per file figures below are collected tests, so they sum to the headline exactly**, and that is the check: `pytest --collect-only -q` prints a count per file, and the total must equal the number above. They are not `def test_` lines, which come to 2788, because a parametrised case is one line and several tests.
...
| `routers/test_settings.py` | 95 | Feature flags, the masked API key, the overdue webhook settings, admin-only writes |
