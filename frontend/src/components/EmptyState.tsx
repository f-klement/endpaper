...
export default function EmptyState({ icon, title, hint }: EmptyStateProps) {
  return (
    <div className="flex flex-col items-center text-center py-16 px-6">
      ...
      <span className="mb-4 grid place-items-center w-11 h-11 rounded-full bg-paper-200/60 text-paper-500 dark:bg-paper-800 dark:text-paper-400">
        <Icon name={icon} className="w-5 h-5" />
      </span>
      <p className="font-medium text-paper-800 dark:text-paper-100">{title}</p>
      {hint && (
        <p className="text-sm mt-1.5 max-w-xs text-paper-500 dark:text-paper-400">
          {hint}
        </p>
      )}
    </div>
