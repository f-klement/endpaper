 ...
 *
 * Settings is an index of six routes since 2026-08-27. It held thirteen
 * collapsible sections across one file, and the fold was doing a route's job.
 *
 * So what is asserted here is the map and nothing else: six entries, in the
 ...
 * of this page**, so their absence is a failure rather than a cosmetic one: six
 ...
 * this page reads: three of the six entries are admin only, and under proxy
...
/** What is left when the three admin only routes are dropped. */
const MEMBER_HEADINGS = ["Appearance", "Your library", "About Endpaper"];

describe("SettingsPage", () => {
  it("is six links and nothing else for an admin", () => {
    render(true);

    expect(screen.getAllByRole("link")).toHaveLength(6);
    expect(SETTINGS_ROUTES).toHaveLength(6);
  });
  ...
  it("offers a member only the screens that hold something for them", () => {
    ...
    // refusing. Six links with no mark would turn that into three dead ends, a
    // tap apart, each advertised with a sentence promising content.
    render(false);

    expect(
      screen.getAllByRole("link").map((link) => link.getAttribute("href")),
    ).toEqual(["/settings/appearance", "/settings/library", "/settings/about"]);
    for (const heading of MEMBER_HEADINGS) {
      expect(
  ...
  it("does not read the cached account", () => {
    ...
    // admin always: reading it dropped three entries off their own index. The
  ...
  it("filters its own copy and leaves the table alone", () => {
    // The flag decides what is offered, never what is allowed, so the three
    // marked routes are still in the table the router is written against.
    // That they are still *mounted* is a different claim and is asserted where
    // the router's source is actually read: `types.test.ts::has a route
    // mounted at every path it lists`.
    expect(SETTINGS_ROUTES.filter((route) => route.adminOnly)).toHaveLength(3);
    expect(SETTINGS_ROUTES).toHaveLength(6);
  });
  ...
  it("says what is behind every link, not just its name", () => {
    ...
    // property of all six rather than by quoting one sentence, so adding a
    // seventh route with no description fails here.
    render(true);
